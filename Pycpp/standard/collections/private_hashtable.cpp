...
#pragma region Definition : Private HashTable
	...
	void __HashTable::extend_from_hashtable(__HashTable* source)
	{
		...
		for (u32 i = 0u; i < capacity; ++i)
		{
			...
			// Entry "source" không rỗng
			auto& entry_head = data[i];

			for (; head;)	// Duyệt "source"
			{
				auto index = index_hashing(head->hash_value);

				// Thêm Entry vào "this"
				if (entry_head == nullptr)
					entry_head = new Entry(*head);
				else
					entry_head = new Entry(*head, entry_head);
			}
		}
	...
	void __HashTable::extend_from_iterable(Iterable* iterable)
	{
		...
		{
			auto entry = new Entry(iter->get());
			auto index = (entry->hash_value & capacity) - 1u;
			auto& entry_head = data[index];
